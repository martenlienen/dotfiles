...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package lsp-mode
    ...
    ;; Do not watch code from installed packages. This regex matches anywhere in the path on
    ;; purpose, because the .pyenv root is recognized as a workspace root and therefore not
    ;; checked itself. Only its subdirectories are checked against the ignored regexes.
