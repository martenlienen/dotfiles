...
class SwapTick(Tick):
    def do(self, context: Context, action_params: List[str]):
        ...
        destination_mark = (
            Mark.previous() if action_params[-1] == "previous" else action_params[-1]
        )
        ...
        if destination is not None:
            mover.swap(
                destination,
                layout.swap_mark_last() if layout is not None else False,
                swap_marks,
            )
...
class LayoutTick(Tick):
    def do(self, context: Context, action_params: List[str]):
        layout = Layouts.create(
            self._action_name, action_params, context.workspace.name
        )
        if layout is not None:
            logger.debug(
                f"  [ipc] tick event - set workspace layout to {self._action_name}"
            )
            self._layouts.add(layout)
            self._state.add_workspace_sequence(context.workspace.name)
            self._state.start_rebuild(
                RebuildCause.layout_change(self._action_name),
                context,
                layout.mark_main(),
                layout.mark_last(),
            )
