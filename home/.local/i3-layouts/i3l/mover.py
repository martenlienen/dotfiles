...
class Mover:
    ...
    def swap(self, destination: Con, swap_mark_last: bool, swap_marks=None):
        ...
        if self._context.workspace_sequence is not None:
            self._context.workspace_sequence.switch_container_order(
                self._context.focused, destination
            )
    ...
    def _destination_candidates(self, direction: str, origin: Con) -> List[Con]:
        def vertical_overlap(candidate: Con):
            return (
                ...
                or candidate.rect.y
                <= origin.rect.y + origin.rect.height
                <= candidate.rect.y + candidate.rect.height
            )
        ...
        def horizontal_overlap(candidate: Con):
            return (
                ...
                or candidate.rect.x
                <= origin.rect.x + origin.rect.width
                < candidate.rect.x + candidate.rect.width
            )

        def overlap():
            return (
                horizontal_overlap if direction in ["up", "down"] else vertical_overlap
            )
        ...
        if direction in ["left", "right"]:
            candidates = [
                con
                for con in self._context.containers
    ...
    def _switch_marks(self, destination: Con, swap_mark_last: bool, swap_marks=None):
        ...
        mark_to_swap = (
            [Mark.MAIN, Mark.LAST] + swap_marks
            if swap_mark_last
            else [Mark.MAIN] + swap_marks
        )
        for mark in destination.marks:
            if Mark.any(mark, mark_to_swap):
                self._context.exec(
                    f'[con_id="{self._context.focused.id}"] mark --add {mark}'
                )
    ...
    def _shortest_distance(cls, origin: Con, containers: List[Con]) -> Optional[Con]:
        ...
        for con in containers:
            ...
            if dist < shortest_dist and not (
                origin.rect.x == con.rect.x and origin.rect.y == con.rect.y
            ):
                destination = con
                shortest_dist = dist
        return destination
