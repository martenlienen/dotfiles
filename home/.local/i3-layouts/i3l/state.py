...
from i3ipc import CommandReply, Con, Connection, TickReply
...
def is_layout_container(container: Con) -> bool:
    return (
        container.window is not None
        and container.type == "con"
        and not is_floating_container(container)
    )
...
class WorkspaceSequence:
    ...
    def get_order(self, con_id: int) -> Optional[int]:
        return (
            self._container_orders[con_id] if con_id in self._container_orders else None
        )
    ...
    def set_stale(self, stale: bool, con_id: int = 0):
        ...
        if (
            con_id == 0
            or self.stale_con_id == 0
            or self._container_orders[con_id]
            < self._container_orders[self.stale_con_id]
        ):
...
class Context:
    ...
    def contains_container(self, con_id: int) -> bool:
        containers = [
            container for container in self.containers if container.id == con_id
        ]
    ...
    def sorted_containers(self) -> List[Con]:
        ...
        return sorted(
            self.containers,
            key=lambda container: self.workspace_sequence.get_order(container.id) or n,
        )
    ...
    def xdo_map_window(self, rebuild_container: RebuildContainer):
        ...
        command = shlex.split(
            f"xdotool windowsize {window_id} {rebuild_container.width} {rebuild_container.height} "
    ...
    def _sync_containers(cls, workspace: Con) -> List[Con]:
        containers = [
            container for container in workspace if is_layout_container(container)
        ]
        return sorted(containers, key=lambda container: container.window)

    @staticmethod
    def _sync_workspace_sequence(
        containers: List[Con], workspace_sequence: WorkspaceSequence
    ) -> WorkspaceSequence:
...
class RebuildAction:
    ...
    def start_rebuild(
        ...
        if len(containers) == 0 or (
            con_id != 0 and not context.workspace_sequence.contains(con_id)
        ):
            self.end_rebuild(context)
            return

        self.containers_to_recreate = self._containers_after(
            con_id, containers, context.workspace_sequence
        )
...
class State:
    ...
    def start_rebuild(
        ...
        self.rebuild_action.start_rebuild(
            context, rebuild_cause, main_mark, last_mark, con_id
        )
    ...
    def is_last_container_rebuilt(self, container: Con):
        return (
            self.rebuild_action.last_container_rebuilt is not None
            and self.rebuild_action.last_container_rebuilt.window == container.window
        )

    def get_workspace_sequence(
        self, workspace_name: str
    ) -> Optional[WorkspaceSequence]:
        return (
            self.workspace_sequences[workspace_name]
            if workspace_name in self.workspace_sequences
            else None
        )
