...
modifying the variable values."
  (setq-default
   ...
   ;; with a supported type is opened). Possible values are `all', `unused' and
   ;; `nil'. `unused' will lazy install only unused layers (i.e. layers not
   ...
   ;; `dotspacemacs-configuration-layers' to install it. (default 'unused)
   dotspacemacs-enable-lazy-installation 'unused
   ;; If non-nil then Spacemacs will ask for confirmation before installing a
   ;; layer lazily. (default t)
   dotspacemacs-ask-for-lazy-installation t
   ;; If non-nil layers with lazy install support are lazy installed. List of
   ;; additional paths where to look for configuration layers. Paths must have a
   ;; trailing slash (i.e. `~/.mycontribs/')
   ...
   '(better-defaults
     ...
     auto-completion
     (git :packages (not evil-magit)
          :variables magit-commit-ask-to-stage nil)
     (shell :variables
     ...
     ;; A very simple reStructuredText layer
     rst

     ;; A layer for my dict.cc package
     dictcc

     ;; Some short but useful commands
     cqql)
...
any user code in there besides modifying the variable values."
  ...
  (setq-default
   ...
   ;; `insert state' is replaced by the `hybrid state' with `emacs' key
   ;; bindings. The value can also be a list with `:variables' keyword (similar
   ...
   ;; `random' chooses a random text banner in `core/banners' directory. A
   ;; string value must be a path to an image format supported by your Emacs
   ;; build. If the value is nil then no banner is displayed. (default
   ;; 'official)
   ...
   ;; list-type are: `recents' `bookmarks' `projects' `agenda' `todos'." List
   ;; sizes may be nil, in which case `spacemacs-buffer-startup-lists-length'
   ;; takes effect.
   ...
   ;; The leader key accessible in `emacs state' and `insert state' (default
   ;; "M-m")
   ...
   ;; If non-nil, J and K move lines up and down when in visual mode. (default
   ;; nil)
   ...
   ;; stored in the cache directory and `nil' to disable auto-saving. (default
   ;; 'cache)
   ...
   ;; source settings. Else, disable fuzzy matching in all sources. (default
   ;; 'always)
   ...
   ;; commands bound to the current keystroke sequence. (default 0.4)
   ...
   ;; If non nil a progress bar is displayed when spacemacs is loading. This may
   ;; increase the boot time on some systems and emacs builds, set it to nil to
   ;; boost the loading time. (default t)
   ...
   ;; if `dotspacemacs-fullscreen-at-startup' is nil. (default nil) (Emacs 24.4+
   ;; only)
   ...
   ;; Control line numbers activation. If set to `t' or `relative' line numbers
   ;; are turned on in all `prog-mode' and `text-mode' derivatives. If set to
   ;; `relative', line numbers are relative. This variable can also be set to a
   ;; property list for finer control:
   ;; 
   ;; '(:relative nil
   ...
   ;; specified with an installed package. Not used for now. (default nil)
...
`dotspacemacs/user-config' first."
  (setq
   ...
   ;; options by running ~ssh~ against the host ~host.does.not.exist~ which leads
   ;; to a wait until timeout if you are connected to a SpeedPort router. This
   ;; can be prevented by setting the options before loading tramp.
   ...
   frame-title-format '("" invocation-name ": %b - " mode-name)
   ;; Ensure that files end with a newline
   require-final-newline t
   ;; Disable backup files and autosaves
   backup-inhibited t
   auto-save-default nil
   ;; Yank at point instead of where I click
   mouse-yank-at-point t
   ;; Set this to an empty string so that it uses an interactive shell instead
   ;; of a login shell because that would not read my .zshenv.
...
package is loaded, you should place your code here."
  ...
  ;; use-cases. But ~org-mode~ comes to our rescue. Simply enabling
  ;; ~org-table-mode~ gives you automatically resizing ASCII tables everywhere
